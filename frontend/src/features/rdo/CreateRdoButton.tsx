...
export function CreateRdoButton({ values }: { values: TimeEntryFormValues }) {
  ...
  return <section className="border-t ui-border pt-5">
    <button type="button" disabled={busy || !profile} onClick={() => void create()} className="rounded-xl border ui-border px-4 py-2.5 text-sm font-bold ui-text transition hover:border-[var(--color-primary)] hover:text-[var(--color-primary)] disabled:opacity-60">{busy ? 'Gerando RDO…' : 'Gerar RDO'}</button>
    <p className="mt-2 text-xs ui-text-subtle">Gera um PDF com os dados atuais, sem salvar o apontamento.</p>
