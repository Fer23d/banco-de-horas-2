...
export function BalancePeriodFilter({ startDate, endDate, isCustomRange, onChange, onApply, onClear, defaultPeriodLabel = 'Usar mês do calendário' }: BalancePeriodFilterProps) {
  ...
  return (
    <form onSubmit={submit} className="grid gap-3 rounded-2xl border ui-border ui-surface p-4 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_auto_auto]" aria-label="Selecionar período dos saldos">
      ...
      <button type="button" onClick={onClear} disabled={!isCustomRange} className="self-end rounded-xl border ui-border px-4 py-2.5 text-sm font-bold ui-text disabled:opacity-40">{defaultPeriodLabel}</button>
    </form>
  )
}
