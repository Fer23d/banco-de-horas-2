...
import { activityOptionsByWorkContext } from '../time-entries/domain'
...
export function HistoryFilters({ value, onChange, onApply }: { value: HistoryFiltersValue; onChange: (value: HistoryFiltersValue) => void; onApply: () => void }) {
  ...
  return (
    <form onSubmit={(event) => { event.preventDefault(); onApply() }} className="grid gap-4 rounded-2xl border ui-border ui-surface p-5 md:grid-cols-2 xl:grid-cols-4" aria-label="Filtros do histórico">
      ...
      <div><label htmlFor="history-activity" className="text-sm font-bold ui-text">Atividade</label><select id="history-activity" value={value.activityId} onChange={(event) => set('activityId', event.target.value)} className={fieldClassName}><option value="">Todas</option>{activityOptions.map((activity) => <option key={activity.id} value={activity.id}>{activity.name}</option>)}</select></div>
      <div><label htmlFor="history-discipline" className="text-sm font-bold ui-text">Disciplina</label><select id="history-discipline" value={value.disciplineCode} onChange={(event) => set('disciplineCode', event.target.value as HistoryFiltersValue['disciplineCode'])} className={fieldClassName}><option value="">Todas</option><option value="C">Campo</option></select></div>
